...
void EffectBouncingPhysics::renderFrame() {
  ...
  const float speed = 0.12f + speed01 * (2.2f + 1.6f * level01);
  const uint8_t balls = static_cast<uint8_t>(1 + static_cast<uint8_t>(level01 * 5.0f)); // 1..6
  ...
  for (uint8_t outIdx = 0; outIdx < led.outputCount(); ++outIdx) {
    ...
    for (uint16_t px = 0; px < out.ledCount; ++px) {
      ...
      for (uint8_t b = 0; b < balls; ++b) {
        ...
        const float width = 0.02f + 0.05f * level01 + 0.008f * b;
        const float dist = fabsf(x - center);
        if (dist <= width) {
          const float n = 1.0f - (dist / width);
          const float intensity = smoothstep(0.0f, 1.0f, n);
          const uint32_t ball = scaleColorFloat(s.primaryColors[b % 3], intensity * gain * (0.7f + 0.3f * level01));
